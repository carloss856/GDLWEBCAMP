...
foreach($pedidExtra as $key => $value){
    if( (int) $value['cantidad'] > 0 ) {
        if( $key === 'camisas') {
...
$redireccionar->setReturnUrl(URL_SITIO . "/pago_finalizado.php?&id_pago={$id_registros}")
              ->setCancelUrl(URL_SITIO . "/pago_finalizado.php?&id_pago={$id_registros}");
